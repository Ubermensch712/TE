...
# ==========================================
# bloque 1: generador del sistema
# ==========================================

# esta función se encarga exclusivamente de la creación de las series temporales
...
# ==========================================
# bloque 2: metricas de informacion
# ==========================================

# motor de simbolización de bandt-pompe
...
# implementación de la entropía de transferencia simbólica parcial
function fast_pste(s_src, s_tar, s_cnd)
    ...
    h_func(counts) = (tot = sum(counts); tot == 0 ? 0.0 : -sum([c > 0 ? (p=c/tot; p*log2(p)) : 0.0 for c in counts]))
    return h_func(c_c) + h_func(c_xyz) - h_func(c_f) - h_func(c_yz)
end

# implementación del índice de causalidad de granger condicional
function fast_cgci(src, tar, cnd, p=2)
    n_total = length(tar)
    y_vec = tar[p+1:n_total]
    n_obs = length(y_vec)
    x_r = ones(n_obs, 1 + 2*p)
    for i in 1:p
        x_r[:, 1+i] = tar[p+1-i : n_total-i]
        x_r[:, 1+p+i] = cnd[p+1-i : n_total-i]
    end
    x_f = hcat(x_r, zeros(n_obs, p))
    for i in 1:p
        x_f[:, 1+2*p+i] = src[p+1-i : n_total-i]
    end
    var_r = var(y_vec - x_r * (x_r \ y_vec))
    var_f = var(y_vec - x_f * (x_f \ y_vec))
    return (var_f > 0 && var_r > var_f) ? log(var_r / var_f) : 0.0
end

# ==========================================
# bloque 3: pruebas de significancia
# ==========================================

# validación estadística mediante el método de barajado para pste
...
# validación estadística mediante el método de barajado para cgci
...
# ==========================================
# bloque 4: ejecucion principal (main)
# ==========================================

function main()
    # configuración de la semilla y parámetros de montecarlo
    Random.seed!(123)
    n_realizaciones = 100
    tamanos_n = [512, 2048]

    # acumuladores para los resultados estadísticos
    tabla_pste = zeros(Int, 2, 6)
    tabla_cgci = zeros(Int, 2, 6)

    println("\nejecutando benchmarking sistema 2 (papana et al. 2017)")
    println("-"^70)

    for (idx, n) in enumerate(tamanos_n)
        print("simulando tamano n = $n ... ")
        for i in 1:n_realizaciones
            # llamada a la función de generación
            x_sim1, x_sim2, x_sim3 = generar_sistema_2(n)
            
            # llamada a las funciones de métricas y simbolización
            s1, s2, s3 = zeros(Int, n), zeros(Int, n), zeros(Int, n)
            simbolizar!(s1, x_sim1); simbolizar!(s2, x_sim2); simbolizar!(s3, x_sim3)
            
            # evaluación de significancia pste
            ...
            # evaluación de significancia cgci
    ...
    for (i, n) in enumerate(tamanos_n)
        @printf("%-7d |   %d   |   %d   |   %d   |   %d   |   %d   |   %d\n", 
                n, tabla_pste[i,1], tabla_pste[i,2], tabla_pste[i,3], tabla_pste[i,4], tabla_pste[i,5], tabla_pste[i,6])
    end
    println("-"^70)
    println("cgci (p=2)")
    for (i, n) in enumerate(tamanos_n)
        @printf("%-7d |   %d   |   %d   |   %d   |   %d   |   %d   |   %d\n", 
                n, tabla_cgci[i,1], tabla_cgci[i,2], tabla_cgci[i,3], tabla_cgci[i,4], tabla_cgci[i,5], tabla_cgci[i,6])
    end
    println("-"^70)
end
